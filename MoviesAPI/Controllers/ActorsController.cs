...
namespace MoviesAPI.Controllers
{
    ...
    public class ActorsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<ActorsMoviesDTO>>> SearchByName(string query)
        {
            if (string.IsNullOrWhiteSpace(query)) { return new List<ActorsMoviesDTO>();}

            return await context.Actors
                .Where(x => x.Name.Contains(query))
                .OrderBy(x => x.Name)
                .Select(x => new ActorsMoviesDTO { id = x.id, Name = x.Name, Picture = x.Picture })
                .Take(5).ToListAsync();
        }
